<?php
	...
	$quantity = isset($_GET["quantity"]) && is_numeric($_GET["quantity"]) ? intval($_GET["quantity"]) : 25;
	...
	// Truncate Items
	if(isset($_GET["truncate"]) && $_GET["truncate"])
	{
		$items->TruncateTable();
	}
	...
	if(isset($item) || $action=="add") // Show Item
	{
		...
		// If Html fields exists create CKEditor objects
		if($items->HtmlFields)
		{
			//$page["onload"] .= "CKEDITOR.config.contentsCss = '".$page["rootpath"].SMARTY_TEMPLATES_DIR."css/global.css';";
			//$page["onload"] .= "CKEDITOR.config.bodyClass = 'content';";
			$page["onload"] .= "CKEDITOR.config.toolbar = 'Full';";
			$page["onload"] .= "CKEDITOR.config.height = 200;";
			$page["onload"] .= "CKEDITOR.config.toolbarCanCollapse = true;";
			foreach($items->HtmlFields as $key) $page["onload"] .= "CKEDITOR.replace('".$items->DbFields[$key]['field']."');";
		}
