...
class Base
{
  ...
  public $SetFields   = false;
	...
	public function SetItemDbFields()
	{
		...
		# $dbField['key_table'] = 'brands';  - Таблица для связки. Используется для типов - key
		# $dbField['key_alias'] = 'b';  - Alias таблицы для связки. Используется для типов - key
		# $dbField['key_value'] = 'brand_name';  - Имя поля выбираемого по ключу из таблица для связки. Используется для типов - key
    ...
    # $dbField['set_value'] = 'stone_name';  - Имя выбираемого поля из таблица для связки. Используется для типов - set
		# $dbField['set_order'] = 'stone_order';  - Поле по которому будет сортироваться значения. Используется для типов - set
    # $dbField['set_bind_table'] = 'products_stones';  - Имя таблицы для связки ключей. Используется для типов - set
		# $dbField['array'] = '$config["currency"]';  - Имя массива. Используется для типов - array
	...
	{
		foreach($this->DbFields as $key => $value) if($value["field"] == $fieldName) return $key;
	...
	public function SaveItem()
	{
		...
		else // Insert item
		{
			...
			{
				$id = $db->sql_nextid();
				$this->SetInfo("Запись #{$id} добавлена", __FILE__, __LINE__);
	...
	public function GetItem($id)
	{
		...
		if(!$result = $db->sql_query($sql))
		{
			$this->SetError("Ошибка при выборе записи #".$id, __FILE__, __LINE__, $db->sql_error());
			return false;
		}
		elseif($db->sql_numrows($result))
		{
	...
	public function ToggleField($id, $field)
	{
		...
		elseif($db->sql_numrows($result))
		{
			...
			{
				$this->SetInfo("Запись #{$id}: поле {$field} изменено на ".$fieldValue, __FILE__, __LINE__);
				return $fieldValue;
			}
		}
	...
	public function DeleteItem($id)
	{
		...
		elseif($db->sql_affectedrows())
		{
			$this->SetInfo("Запись #{$id} удалена", __FILE__, __LINE__);
	...
	public function TruncateTable()
	{
		...
		if(!$result = $db->sql_query($sql))
		{
			$this->SetError("Ошибка при очищении всех записей", __FILE__, __LINE__, $db->sql_error());
			return false;
		}
		else
		{
			$this->SetInfo("Все записи очищены", __FILE__, __LINE__);
			//if($this->ItemImagePath) $this->DeleteItemImage($this->GetItemImage($this->ItemImagePrefix.$id));
			return true;
		}
	...
	public function ResizeImage($sourceFile, $targetFile, $width, $height, $resizeIfSmaller=false, $maintainAspectRatio=true, $fixedResize=false)
	{
		...
		$imgTransformer->jpegOutputQuality = 80;
	...
	public function WatermarkImage($imageFile, $watermarkImage = "", $valign = "", $halign = "")
	{
		...
		$imageExt = strtoupper(substr($imageFile, strrpos($imageFile, ".") + 1)); // File extension
	 ...
	 *
	 *  @param  string   $sourceFiles          Source images files (../files/images/product-*.*)
	 *  @param  string   $watermarkImage       Watermark Image file (../templates/img/logo.png)
	 *  @param  string   $valign               Vertical align (LEFT | CENTER | RIGHT)
	 *  @param  string   $halign               Horizontal align (LEFT | CENTER | RIGHT)
	 *
	 ...
	 *
	 */
	public function WatermarkAllImages($sourceFiles, $watermarkImage = "", $valign = "", $halign = "")
	{
		## EXAMPLE: $this->WatermarkAllImages($this->ItemImagePath."original/*.*");
		if(!$maskImages = glob($sourceFiles)) return false;
		...
		for($i=0; $i<count($maskImages); $i++)
		{
			//if($i<100) continue;
			$imgName = substr($maskImages[$i], strrpos($maskImages[$i], "/") + 1);
			$this->WatermarkImage($maskImages[$i], $watermarkImage, $valign, $halign);
			$this->SetInfo("Печать успешно добавлена в файл ".$imgName, __FILE__, __LINE__);
		}
